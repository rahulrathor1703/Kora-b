...
export class ContactListsService {
  ...
  async getImportableFields(organizationId: string) {
    const tableColumns = await this.formsService.resolveTableColumns(
      CONTACT_LIST_IMPORT_FORM_KEY,
      organizationId,
    );
  ...
  async importFromMappedRows(
    ...
    const resolvedOrganizationId = requireOrganizationId(organizationId);
    this.assertUniqueSourceColumns(fieldMapping);

    const tableColumns = await this.formsService.resolveTableColumns(
      CONTACT_LIST_IMPORT_FORM_KEY,
      resolvedOrganizationId,
    );
    this.assertFieldMappingMatchesTableColumns(fieldMapping, tableColumns);

    const mappedRows = this.mapRows(rows, fieldMapping);
    const members = this.deduplicateMembers(mappedRows);
    const skippedCount = rows.length - members.length;
    ...
    try {
      await this.contactListsRepository.importListWithMembers(
        resolvedOrganizationId,
        name.trim(),
        members,
        fieldSchema,
