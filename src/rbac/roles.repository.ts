...
export class RolesRepository {
  ...
  async addPermissionToRolesHaving(
    sourcePermissionId: string,
    targetPermissionId: string,
  ): Promise<void> {
    const rolesWithSource = await this.rolePermissionRepository.find({
      where: { permissionId: sourcePermissionId },
    });
    const roleIds = [...new Set(rolesWithSource.map((entry) => entry.roleId))];
