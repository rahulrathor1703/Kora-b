...
export class CompaniesService {
  ...
  async updateFieldSchema(
    ...
    if (deletedKeys.includes(CATEGORY_FIELD_KEY)) {
      throw new BadRequestException('Category field cannot be deleted');
    }
  ...
  async update(
    ...
    if (dto.brokerName !== undefined) {
      const brokerName = dto.brokerName.trim();
      if (!brokerName) {
        throw new BadRequestException('Broker name is required');
      }
  ...
  private validateFieldSchema(
    ...
    const hasCategory = normalized.some(
      (field) => field.key === CATEGORY_FIELD_KEY && field.system,
    );
    ...
    if (!hasCategory) {
      throw new BadRequestException(
        'Schema must include system field categoryId',
      );
    }
